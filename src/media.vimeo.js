...
/*
 * modified to facilitate private video functionality for ProofMe
 */

define (["foreign/video-js"],function(videojs) {
    /**
    ...
    */
    ...
    videojs.Vimeo.prototype.src = function(src){
            ...
            var srcUrl = this.baseUrl + this.videoId + '?' + videojs.Vimeo.makeQueryString(params);
            if (!this.el_.src || this.el_.src !== srcUrl) {
                // prevent video reloading if src isn't different (get's called a 2nd time from videojs ready event) - njh 9/30/15
                $(this.el_).css("display", "none");
                this.el_.src = srcUrl;
            } else {
                $(this.el_).css("display", "block");
            }
    };
    ...
    videojs.Vimeo.prototype.onLoad = function(){
        ...
        this.inputScreenTimer = false;
        this.inputScreenLast = false;
        ...
        this.vimeo.addEvent('ready', function(id){
            ...
            self.vimeo.addErrorHandler("password_video", function(data) { self.handlePrivate(data); });
            self.vimeo.addErrorHandler("private_video", function(data) { self.handlePrivate(data); });
    ...
    videojs.Vimeo.prototype.handlePrivate = function(data) {
        // only way to tell if vimeo input screen is active is to catch errors
        // routed to this function when trying to invoke api methods
        if (!this.inputScreenTimer) {
            app.content.$videoBlock.hide();
            this.iframeSrc = app.content.$videoIframe.attr("src");
            this.inputScreenTimer = window.setInterval(function() {
                // if the last error was logged more than 750 ms ago, assume they are past the input screen
                if (document.hasFocus() && this.inputScreenLast &&
                        (new Date() - this.inputScreenLast > 750)) {
                    clearInterval(this.inputScreenTimer);
                    this.inputScreenLast = false;
                    app.content.$videoBlock.show();
                } else {
                    this.vimeo.api("pause");
                }
            }.bind(this), 500);
        } else {
            this.inputScreenLast = new Date();
        }
    };
    ...
    var Froogaloop = (function(){
        ...
        Froogaloop.fn = Froogaloop.prototype = {
            ...
            */
            ...
            /*
             * Registers an error handler for specific errors returned from api
             *
             * @param errorName (String): Name of the error to assign handler for.
             * @param callback (Function): Function that should be called for that specific error.
             */
            addErrorHandler: function(errorName, callback) {
        ...
        */
        function onMessageReceived(event) {
            ...
            if (data && data.event === "error" && data.data && data.data.code) {
                if (errorHandlers[data.data.code]) {
                    return errorHandlers[data.data.code](data.data);
                }
            }
